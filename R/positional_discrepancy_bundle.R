# Utilities for the consolidated positional-discrepancy result bundle.

positional_discrepancy_bundle_path <- function() {
  project_path("derived_data", "positional_discrepancy_results.rds")
}

new_positional_discrepancy_bundle <- function() {
  list(schema_version = "1.0", analysis = "positional_discrepancy",
       alignment_method = "approved_common_grid", behaviors = list())
}

validate_positional_discrepancy_bundle <- function(bundle, require_all_behaviors = FALSE) {
  ...
  if (!identical(bundle$analysis, "positional_discrepancy")) stop("Unexpected analysis type.", call. = FALSE)
...
read_positional_discrepancy_bundle <- function(path = positional_discrepancy_bundle_path(), require_all_behaviors = TRUE) {
  if (!file.exists(path)) stop("Results not found: ", path, "\nRun behavior analyses first.", call. = FALSE)
  bundle <- readRDS(path)
  validate_positional_discrepancy_bundle(bundle, require_all_behaviors)
  bundle
}

update_positional_discrepancy_bundle <- function(behavior, paired_data, rmse, flight_metadata,
                                                path = positional_discrepancy_bundle_path()) {
  expected <- c("cubes", "chasing", "foraging", "soaring", "transiting")
  if (!behavior %in% expected) stop("Unknown behavior: ", behavior, call. = FALSE)
  bundle <- if (file.exists(path)) read_positional_discrepancy_bundle(path, FALSE) else new_positional_discrepancy_bundle()
  ...
  validate_positional_discrepancy_bundle(bundle)
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
  saveRDS(bundle, path)
  invisible(path)
}

positional_discrepancy_behavior_result <- function(bundle, behavior) {
