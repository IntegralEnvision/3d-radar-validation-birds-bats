...
load_analysis_config <- function(path = file.path("config", "analysis_config.yml")) {
  ...
  required_top_level <- c(
    ...
    "outputs", "positional_deviation", "detection_rate"
  )
  ...
  offsets <- unlist(config$positional_deviation$radar_y_offsets_m, use.names = TRUE)
  config$positional_deviation$radar_y_offsets_m <- setNames(as.numeric(offsets), names(offsets))
  ...
  if (!setequal(names(config$positional_deviation$radar_y_offsets_m), radar_types)) {
  ...
  if (any(!is.finite(config$positional_deviation$radar_y_offsets_m))) {
